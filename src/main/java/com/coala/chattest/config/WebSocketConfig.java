...
public class WebSocketConfig implements WebSocketConfigurer { // Spring 웹 서버에서 WebSocket을 사용하도록 Configuration을 추가해주는 작업을 진행했다.
    ...
    public void registerWebSocketHandlers(WebSocketHandlerRegistry registry) {
         ...
         * 직접 구현한 웹소켓 핸들러 (webSocketHandler)를 웹소켓이 연결될 때, Handshake 할 주소 (/chat/rooms/번호)와 함께 addHandler 메소드의 인자로 넣어준다.
         */
...
//                .addInterceptors(handshakeInterceptor())
    ...
    public HandshakeInterceptor handshakeInterceptor() {
        System.out.println("WebSocketConfig : HandshakeInterceptor");
